...
const TESTS = [
  {
    ...
    name: 'JSON Format',
    weight: 25,
    prompt: `Analyze this login endpoint. Respond ONLY with JSON: {"summary": "brief description"}
...
async function runTest() {
  ...
  for (const test of TESTS) {
    ...
    if (!result.success) {
      console.log(`❌ ${result.error} (${duration.toFixed(1)}s)`);
    ...
    const status = validation.score >= 80 ? '✅' : validation.score >= 50 ? '⚠️ ' : '❌';
  ...
  console.log(`⏱️  Total time: ${totalTime.toFixed(1)}s (${(totalTime/4).toFixed(1)}s avg per test)`);
  
  return { model: 'llama3.1:8b', score: Math.round(totalScore), results };
}
