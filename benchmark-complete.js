#!/usr/bin/env node
/**
 * Benchmark COMPLET des modèles LLM
 */
...
async function pullModel(model) {
  ...
  try {
    const { stdout } = await execAsync(`ollama pull ${model.name}`, { timeout: 900000 });
...
async function runBenchmark() {
  ...
  for (const model of MODELS_TO_TEST) {
    ...
    for (const test of TESTS) {
      console.log(`\n  📋 Test: ${test.name}`);
      const { response, error } = await queryModel(model, test.prompt);
      
      if (error) {
        console.log(`     ❌ Erreur: ${error}`);
        testResults.push({ test: test.id, score: 0, weightedScore: 0, note: error, response: '' });
        continue;
      }
...
function generateReport(results) {
  let report = `# Benchmark LLM - Résultats Complets\n\n`;
  report += `Date: ${new Date().toLocaleString()}\n\n`;
  
  report += `## 📊 Tableau Comparatif\n\n`;
  report += `| Modèle | Provider | Catégorie | Score Total | Installation |\n`;
  report += `|--------|----------|-----------|-------------|--------------|\n`;
  
  for (const r of results) {
    report += `| ${r.model} | ${r.provider} | ${r.category} | **${r.totalScore}%** | ${r.installTime} |\n`;
  }
  
  report += `\n## 🔍 Détails\n\n`;
  for (const r of results) {
    report += `### ${r.model} (${r.provider})\n\n`;
    report += `- Score total: ${r.totalScore}%\n`;
    report += `- Catégorie: ${r.category}\n`;
    report += `- Taille: ${r.size}\n\n`;
    
    for (const t of r.testResults) {
      report += `- **${t.test}**: ${t.score}% (${t.note})\n`;
    }
    report += `\n`;
  }
  
  const sorted = [...results].sort((a, b) => parseFloat(b.totalScore) - parseFloat(a.totalScore));
  report += `## 🏆 Recommandation\n\n`;
  report += `**Gagnant: ${sorted[0].model}** avec ${sorted[0].totalScore}%\n\n`;
  report += `Modèles à garder:\n- mistral:7b (léger, rapide)\n- qwen2.5-coder:14b (excellent pour le code)\n`;
  if (parseFloat(sorted[0].totalScore) > 65) {
    report += `- ${sorted[0].model} (meilleur score)\n`;
  }
...
(async () => {
  console.log('🚀 Démarrage du benchmark LLM...');
  ...
  const reportPath = path.join(outputDir, 'REPORT-FINAL.md');
